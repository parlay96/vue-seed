<template>
  <div class="pl-menu boxSizing">
    <ul class="menuUl" v-if="dalsa">
      <li class="menuli"
          v-for="(item,index) in data"
          :class="{active: item.state && activeFalse && !plMenuFals}"
          @mousemove="limousemove"
          @mouseout="limouseout"
          :key="index">
        <router-link :to="{ path: item.href}" class="links" :style="{ paddingLeft: isCollapse ? '30px' : '40px' }">
...
<script>
  ...
  export default class plMenu2 extends Vue {
    @State('plMenuFals') plMenuFals
    ...
    urlChange (val) {
      ...
      this.$nextTick(() => {
        ...
        this.data.forEach(item => {
          item.state = false
          if (item.submenu.length === 0 && item.href === this.url) {
            item.state = true
           this.$nextTick(() => {
    ...
    limousemove () {
      this.$store.state.plMenuFals = true
      this.activeFalse = false
    }
    limouseout () {
      this.$store.state.plMenuFals = false
      this.activeFalse = true
    }
    created () {
      this.data.forEach(item => {

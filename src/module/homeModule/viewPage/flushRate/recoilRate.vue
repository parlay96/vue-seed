...
<script>
  ...
  @Component()
  // 复冲率
  export default class recoilRate extends Vue {
    @homeModule.Action('payPayerRateLine') payPayerRateLine
    dateVal = []
    created () {
    ...
    getData () {
      this.payPayerRateLine({start_time: this.dateVal[0], end_time: this.dateVal[1]}).then(data => {
        if (data.code === 0) {

...
<script>
  ...
  @Component()
  // 次日留存
  export default class nextDay extends Vue {
    @homeModule.Action('retentionOne') retentionOne
    dateVal = []
    created () {
    ...
    getData () {
      this.retentionOne({start_time: this.dateVal[0], end_time: this.dateVal[1]}).then(data => {
        if (data.code === 0) {

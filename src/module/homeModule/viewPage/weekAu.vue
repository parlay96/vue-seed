...
<script>
  ...
  @Component()
  // 周活
  export default class weekLine extends Vue {
    @homeModule.Action('weekHuoLine') weekHuoLine
    dateVal = []
    created () {
    ...
    getData () {
      this.weekHuoLine({start_time: this.dateVal[0], end_time: this.dateVal[1]}).then(data => {
        if (data.code === 0) {
          let legendData = []
          let seriesData = []

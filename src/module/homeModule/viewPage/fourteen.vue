...
<script>
  ...
  @Component()
  // 十四日留存
  export default class fourteen extends Vue {
    @homeModule.Action('retentionFourteen') retentionFourteen
    dateVal = []
    created () {
      let date = cycleDate('近14天')
      if (date && date.length) {
    ...
    getData () {
      this.retentionFourteen({start_time: this.dateVal[0], end_time: this.dateVal[1]}).then(data => {
        if (data.code === 0) {

...
<script>
  ...
  @Component()
  // 每日新增
  export default class dayAdd extends Vue {
    @homeModule.Action('dayAddLine') dayAddLine
    dateVal = []
    created () {
    ...
    getData () {
      this.dayAddLine({start_time: this.dateVal[0], end_time: this.dateVal[1]}).then(data => {
        if (data.code === 0) {
          let legendData = []
          let seriesData = []
